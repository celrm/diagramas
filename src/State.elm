  ...
  case msg of
      ...
      ( { model
        | serie =
          if model.sch
            then List.map
                  ( \x ->
                      ( -x + 2 * ( Maybe.withDefault 0 (List.head model.serie) ) )
                      % ( List.length model.serie )
                  )
                  model.serie
            else List.map
                  ( \x ->
                      ( -x % ( List.length model.serie ) )
                  )
                  model.serie
        }
      , Cmd.none)
    ...
    Retrogradar ->
      ( { model
        | serie =
          if model.sch
            then List.reverse model.serie
            else Maybe.withDefault 0 (List.head model.serie) ::
